...
9. **JSON Response format**: Return a JSON array of objects. Each object in the array must map to the 15 schema keys: [created_at, name, email, country_code, mobile_without_country_code, company, city, state, country, lead_owner, crm_status, crm_note, data_source, possession_time, description].
10. **Output constraint**: Do not include any markdown styling, explanation, or wrap the JSON in backticks. Return the raw JSON string array only.

### Raw CSV Records to Map:
\`\`\`json
${serializedRows}
\`\`\`
`;
